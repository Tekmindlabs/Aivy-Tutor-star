...
class Mem0Bridge:
    def __init__(self):
        ...
        try:
            ...
            # Initialize memory system using Memory instead of Mem0AI
    ...
    def get_all_memories(self, user_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing success status and memories/error
        """
        try:
            self._check_memory_initialized()
            ...
            logger.error(f"Get all memories error: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    try:
        ...
        if command == "search":
            ...
            if "userId" not in args:
                raise ValueError("Missing required argument userId")
            result = bridge.get_all_memories(
                user_id=args["userId"]
            )
        else:
            result = {
